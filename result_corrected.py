...
class Ui_ResultWindow(object):
    def giveAvg(self):  # the function was defined in the wrong place
        a=self.space1.text()
        b=self.space2.text()
        c=self.space3.text()
        d=self.space4.text()
        e=self.space5.text()
        # logic was wrong, we need to calculate total percetage and not avga
        # so the avg will be divided by 500 as in total marks
    #msg.setIcon(QMessageBox)
        avg = int(a)+int(b)+int(c)+int(d)+int(e)
        avg = (avg/500)*100
        #avg = avg*100
        print(avg)
        self.percentspace.setPlainText(str(avg))
    ...
    def setupUi(self, ResultWindow):
        ...
        self.result.clicked.connect(self.giveAvg)
        ...
        #ResultWindow.setCentralWidget(self.centralwidget) # didn't quite understand the use of this line, it was causing error so i commented it
...
if __name__ == '__main__':  # the main function was wrong
    import sys
